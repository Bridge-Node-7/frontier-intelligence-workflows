#!/usr/bin/env python3
"""Regression tests for FIW release semantics and repository safeguards."""
...
class ReleaseSemanticsTests(unittest.TestCase):
    ...
    def test_12_exact_release_marker_rejects_version_substring_spoof(self) -> None:
        with tempfile.TemporaryDirectory(prefix="fiw-version-spoof-") as temporary:
            ...
            current_marker = "**v0.9.0 — Frontier Signal Integrity**"
            self.assertIn(current_marker, text)
            readme.write_text(
                text.replace(current_marker, "**v10.9.01 — Frontier Signal Integrity**", 1),
                encoding="utf-8",
                newline="\n",
            )
            report = validate(repo, check_manifest=False)
    ...
    def test_15_release_builder_check_flag_is_optional(self) -> None:
