...
from frontier_signal_integrity import evaluate_case, structural_issues  # noqa: E402

CASE_PATH = ROOT / "profiles/frontier-signal-integrity/fixtures/valid/assessment-ready.json"
...
class RadiantGuardianTests(unittest.TestCase):
    ...
    def test_01_valid_synthetic_has_no_findings(self):
        ...
        self.assertEqual(report["validation_status"], "NO_FSI_FINDINGS")
    ...
    def test_02_signal_claim_collapse(self):
        ...
        self.assertIn("FSI-SIGNAL-CLAIM-COLLAPSE", self.ids(evaluate(c)))

    def test_03_independence_overstated(self):
        c=load_case(); c["genealogy"]["declared_independent_root_count"]=3
        self.assertIn("FSI-INDEPENDENCE-OVERSTATED", self.ids(evaluate(c)))

    def test_04_anchor_extension_requires_bridge(self):
        c=load_case(); c["reasoning_patterns"]=[{"pattern":"SUPPORTED_ANCHOR_UNSUPPORTED_EXTENSION","claim_reference":"X","rationale":"x","bridge_evidence_references":[],"discriminating_test_ids":["SYN-TEST-01"]}]
        self.assertIn("FSI-ANCHOR-EXTENSION-BRIDGE-MISSING", self.ids(evaluate(c)))
    ...
    def test_10_update_receipt_complete(self):
        ...
        self.assertIn("FSI-UPDATE-RECEIPT-INCOMPLETE", self.ids(evaluate(c)))

    def test_11_upstream_pi_unresolved(self):
        c=load_case(); c["pi_reference"]["validation_sha256"]="0"*64
        self.assertIn("FSI-UPSTREAM-PI-UNRESOLVED", self.ids(evaluate(c)))
    ...
    def test_12_public_boundary_propagates(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            q=ROOT/"profiles/frontier-signal-integrity/fixtures/valid/_temp_pi.json"
            try:
                ...
                self.assertIn("FSI-PUBLIC-BOUNDARY-VIOLATION", self.ids(evaluate(c)))
            finally:
    ...
    def test_13_proof_loop_requires_reassessment_and_learning(self):
        ...
        self.assertIn("FSI-PROOF-LOOP-INCOMPLETE", self.ids(evaluate(c)))
    ...
    def test_15_rg_syn_001_is_expected_review_case(self):
        ...
        self.assertIn("FSI-UPSTREAM-PI-REVIEW-REQUIRED",self.ids(report))
        self.assertIn("FSI-ANCHOR-EXTENSION-BRIDGE-MISSING",self.ids(report))
        self.assertIn("FSI-INDEPENDENCE-OVERSTATED",self.ids(report))
    ...
    def test_16_cli_writes_outside_repo_and_returns_zero_for_valid(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cp=subprocess.run([sys.executable,str(SCRIPTS/"run_frontier_signal_integrity.py"),"--root",str(ROOT),"--case","profiles/frontier-signal-integrity/examples/FSI-SYN-002/case.json","--evaluated-at","2026-09-01T23:59:00Z","--json-output",str(out)],capture_output=True,text=True)
            self.assertEqual(cp.returncode,0,cp.stderr+cp.stdout)
            self.assertEqual(json.loads(out.read_text())["validation_status"],"NO_FSI_FINDINGS")
