...
- `NO_FSI_FINDINGS` — no configured deterministic FSI frontier-signal integrity rule produced a finding.
- `FSI_REVIEW_REQUIRED` — one or more configured FSI integrity rules produced findings.

`NO_FSI_FINDINGS` does not establish that a frontier claim is true, independently verified, safe, or authorized for action. FSI cannot downgrade upstream Perception Integrity findings.
